...
function varargout = wmh_ud2_scripts_getLatVent (ud2param, img, T1, outputFolder)
...
% img register to T1
rimg = ud2_spmbatch_coregistration (ud2param, img, T1, outputFolder);

% T1 segmentation
[cGM,cWM,cCSF,rcGM,rcWM,rcCSF,mat] = ud2_spmbatch_segmentation (ud2param,T1);

% T1 to DARTEL
template1 = fullfile(ud2param.dirs.ud2,'templates','DARTEL_0to6_templates','65to75','Template_1.nii');
template2 = fullfile(ud2param.dirs.ud2,'templates','DARTEL_0to6_templates','65to75','Template_2.nii');
template3 = fullfile(ud2param.dirs.ud2,'templates','DARTEL_0to6_templates','65to75','Template_3.nii');
template4 = fullfile(ud2param.dirs.ud2,'templates','DARTEL_0to6_templates','65to75','Template_4.nii');
template5 = fullfile(ud2param.dirs.ud2,'templates','DARTEL_0to6_templates','65to75','Template_5.nii');
template6 = fullfile(ud2param.dirs.ud2,'templates','DARTEL_0to6_templates','65to75','Template_6.nii');

% run DARTEL to get flow map
flowMap = ud2_spmbatch_runDARTELe (ud2param, ...
                                    rcGM, rcWM, rcCSF, ...
                                    template1, template2, template3, template4, template5, template6);

% bring ventricle to T1 space
copyfile (fullfile (ud2param.dirs.ud2,'templates',...
                    'DARTEL_ventricle_distance_map','DARTEL_ventricle_65to75.nii'),...
          fullfile (outputFolder,'dartel_vent.nii'));

T1space_vent = ud2_spmbatch_DARTELtoNative (ud2param, ...
                                             fullfile (outputFolder,'dartel_vent.nii'), ...
                                             flowMap, ...
                                             'NN');
...
ud2_scripts_writeNii  (ud2param, ...
                        spm_vol(t1SpcDim_vent), ...
                        vent_dat, ...
                        fullfile(outputFolder, 'ventricular_mask.nii'));

% T1 to native space
ventricle_native = ud2_scripts_revReg (ud2param, img, T1, fullfile(outputFolder, 'ventricular_mask.nii'));
...
ud2_scripts_writeNii (ud2param, spm_vol(ventricle_native), ventricle_native_dat, fullfile(outputFolder,'latvent.nii'));
