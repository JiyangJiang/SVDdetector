...
switch flag
	...
	case 'cns2_ud'
		...
		if ud2param.exe.verbose
			fprintf ('%s : Using UBO Detector-segmented WMH (flag = ''cns2_ud'').\n', mfilename);
		end
		...
		if ud2param.exe.verbose
			fprintf ('%s : Calling cns2_spmbatch_DARTELtoNative for reverse warping GM/WM masks, GM/WM/CSF probability maps, and brain mask from DARTEL back to native T1 space (subject ID = %s).\n', mfilename, ...
																																							ud2param.lists.subjs{i,1});
		end

		gmmsk_t1spc   = cns2_spmbatch_DARTELtoNative (ud2param, ud2param.templates.gmmsk,   flowmap      );
		wmmsk_t1spc   = cns2_spmbatch_DARTELtoNative (ud2param, ud2param.templates.wmmsk,   flowmap      );
		gmprob_t1spc  = cns2_spmbatch_DARTELtoNative (ud2param, ud2param.templates.gmprob,  flowmap      );
		wmprob_t1spc  = cns2_spmbatch_DARTELtoNative (ud2param, ud2param.templates.wmprob,  flowmap      );
		csfprob_t1spc = cns2_spmbatch_DARTELtoNative (ud2param, ud2param.templates.csfprob, flowmap      );
		brnmsk_t1spc  = cns2_spmbatch_DARTELtoNative (ud2param, ud2param.templates.brnmsk,  flowmap, 'NN');

		% reslice t1spc images to t1 dimension.
		% cns2_spmbatch_DARTELtoNative only estimate, doesn't reslice.
		% those need further cns2_scripts_revReg will reslice in cns2_scripts_revReg.
		if ud2param.exe.verbose
			fprintf ('%s : Calling cns2_spmscripts_reslice to reslice reverse-warped GM/WM masks, GM/WM/CSF probability maps, and brain mask to native T1 space (subject ID = %s).\n', mfilename, ...
																																							ud2param.lists.subjs{i,1});
		end
		gmmsk_t1spc_resliced   = cns2_spmscripts_reslice (ud2param, t1, gmmsk_t1spc,   1);
		wmmsk_t1spc_resliced   = cns2_spmscripts_reslice (ud2param, t1, wmmsk_t1spc,   1);
		gmprob_t1spc_resliced  = cns2_spmscripts_reslice (ud2param, t1, gmprob_t1spc,  1);
		wmprob_t1spc_resliced  = cns2_spmscripts_reslice (ud2param, t1, wmprob_t1spc,  1);
		csfprob_t1spc_resliced = cns2_spmscripts_reslice (ud2param, t1, csfprob_t1spc, 1);
		brnmsk_t1spc_resliced  = cns2_spmscripts_reslice (ud2param, t1, brnmsk_t1spc,  0);
		...
		if ud2param.exe.verbose
			fprintf ('%s : Calling cns2_scripts_revReg for reverse registering GM/WM masks, GM/WM/CSF probability maps, and brain mask from native T1 to native FLAIR space (subject ID = %s).\n', mfilename, ...
																																							ud2param.lists.subjs{i,1});
		end

		gmmsk_flairSpc   = cns2_scripts_revReg (ud2param, flair, t1, gmmsk_t1spc,   'Tri');
		wmmsk_flairSpc   = cns2_scripts_revReg (ud2param, flair, t1, wmmsk_t1spc,   'Tri');
		gmprob_flairSpc  = cns2_scripts_revReg (ud2param, flair, t1, gmprob_t1spc,  'Tri');
		wmprob_flairSpc  = cns2_scripts_revReg (ud2param, flair, t1, wmprob_t1spc,  'Tri');
		csfprob_flairSpc = cns2_scripts_revReg (ud2param, flair, t1, csfprob_t1spc, 'Tri');
		brnmsk_flairSpc  = cns2_scripts_revReg (ud2param, flair, t1, brnmsk_t1spc        );
		...
		if ud2param.exe.verbose
			fprintf ('%s : Calling cns2_scripts_mask to apply brain mask to native T1 and FLAIR (subject ID = %s).\n', mfilename, ud2param.lists.subjs{i,1});
		end

		cns2_scripts_mask  (ud2param, flair, brnmsk_flairSpc, ...
							fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'flair_brn.nii'));
		cns2_scripts_mask  (ud2param, t1, brnmsk_t1spc, ...
							fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 't1_brn.nii'));
		...
		movefile (gmmsk_t1spc,     fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'gmmsk_t1spc.nii'     ));
		movefile (wmmsk_t1spc,     fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'wmmsk_t1spc.nii'     ));
		movefile (gmprob_t1spc,    fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'gmprob_t1spc.nii'    ));
		movefile (wmprob_t1spc,    fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'wmprob_t1spc.nii'    ));
		movefile (csfprob_t1spc,   fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'csfprob_t1spc.nii'   ));
		movefile (brnmsk_t1spc,    fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'brnmsk_t1spc.nii'    ));
		movefile (gmmsk_flairSpc,  fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'gmmsk_flairSpc.nii'  ));
		movefile (wmmsk_flairSpc,  fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'wmmsk_flairSpc.nii'  ));
		movefile (gmprob_flairSpc, fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'gmprob_flairSpc.nii' ));
		movefile (wmprob_flairSpc, fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'wmprob_flairSpc.nii' ));
		movefile (csfprob_flairSpc,fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'csfprob_flairSpc.nii'));
		movefile (brnmsk_flairSpc, fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'brnmsk_flairSpc.nii' ));

		% update ud2param
		if ud2param.exe.verbose
			fprintf ('%s : Updading ud2param.templates to use FLAIR-space GM/WM/brain masks and GM/WM/CSF probability maps (subject ID = %s).\n', mfilename, ud2param.lists.subjs{i,1});
		end
		ud2param.templates.gmmsk   = fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'gmmsk_flairSpc.nii'  );
		ud2param.templates.wmmsk   = fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'wmmsk_flairSpc.nii'  );
		ud2param.templates.gmprob  = fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'gmprob_flairSpc.nii' );
		ud2param.templates.wmprob  = fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'wmprob_flairSpc.nii' );
		ud2param.templates.csfprob = fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'csfprob_flairSpc.nii');
		ud2param.templates.brnmsk  = fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'brnmsk_flairSpc.nii' );
...
if ud2param.exe.verbose
	fprintf ('%s : Calling cns2_spmbatch_DARTELtoNative for reverse warping ventricular distance map, lobar atlas, and arterial territory atlas from DARTEL back to native T1 space (subject ID = %s).\n', mfilename, ...
																																					ud2param.lists.subjs{i,1});
end

ventdst_t1spc  = cns2_spmbatch_DARTELtoNative (ud2param, ud2param.templates.ventdst,  flowmap      );
lobar_t1spc    = cns2_spmbatch_DARTELtoNative (ud2param, ud2param.templates.lobar,    flowmap, 'NN');
arterial_t1spc = cns2_spmbatch_DARTELtoNative (ud2param, ud2param.templates.arterial, flowmap, 'NN');
...
if ud2param.exe.verbose
	fprintf ('%s : Calling cns2_scripts_revReg for reverse registering ventricular distance map, lobar atlas, and arterial territory atlas from native T1 to native FLAIR space (subject ID = %s).\n', mfilename, ...
																																					ud2param.lists.subjs{i,1});
end

ventdst_flairSpc  = cns2_scripts_revReg (ud2param, flair, t1, ventdst_t1spc,   'Tri');
lobar_flairSpc    = cns2_scripts_revReg (ud2param, flair, t1, lobar_t1spc           );
arterial_flairSpc = cns2_scripts_revReg (ud2param, flair, t1, arterial_t1spc        );
...
movefile (ventdst_t1spc,    fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'ventdst_t1spc.nii'    ));
movefile (lobar_t1spc,      fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'lobar_t1spc.nii'      ));
movefile (arterial_t1spc,   fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'arterial_t1spc.nii'   ));
movefile (ventdst_flairSpc, fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'ventdst_flairSpc.nii' ));
movefile (lobar_flairSpc,   fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'lobar_flairSpc.nii'   ));
movefile (arterial_flairSpc,fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'arterial_flairSpc.nii'));
...
ud2param.templates.ventdst  = fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'ventdst_flairSpc.nii' );
ud2param.templates.lobar    = fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'lobar_flairSpc.nii'   );
ud2param.templates.arterial = fullfile (ud2param.dirs.subjs, ud2param.lists.subjs{i,1}, 'ud', 'preproc', 'arterial_flairSpc.nii');
